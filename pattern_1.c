...
int main() {
    ...
    for (int i = 1; i <= n; i++) {//loop for rows
        ...
        for (int j = 1; j <= i ; j++) {//loop for columns and the loop is running till 2 * i - 1 because the number of digits printing in each volumn is 2 * i - 1 let say row 2 then it will be 2 * 2 - 1 = 3 so we wull be printing 3 digits in the row 2 and fo on
